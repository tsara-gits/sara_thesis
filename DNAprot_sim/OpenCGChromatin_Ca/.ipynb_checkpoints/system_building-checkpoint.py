...
def get_nucleosome_harmonic_bonds(chain_atoms, all_atoms, atom_tree, nucleosome_id, dyad_position, dna_chain, constraints='inner', IDR_d=0.381, IDR_k=8031.):
    ...
    if (dna_chain is not None) and (dyad_position is not None):
        ...
        dna_atoms = [atom for i, atom in enumerate(dna_chain.atoms()) if 'P' not in atom.name]
...
def get_system(positions, topology, globular_indices_dict, dyad_positions, constraints='breathing', mechanics_k_coeff=1.35, overall_LJ_eps_scaling=0.8, protein_DNA_LJ_eps_scaling=1.0, qP=1.0, qAA=0.9, anchor_scaling=1.0, kappa_factor=1.0, debye_length=.79365, coul_rc=None, CoMM_remover=False, periodic=True):
    """
    ...
    ah_string = f'''
    step(rc-r)*(select(step(r-2^(1/6)*sigma), term1, term2) - anchor_flag * exp(-{kappa_factor}*(2.5/sigma)*(max(r - 2^(1/6)*sigma, 0))));
    ...
    epsilon = eps_sf*KH_table(index1, index2, 0);

    anchor_flag = {anchor_scaling} * select(anchor1*dna2 + dna1*anchor2, 1.0, 0.0);
    eps_sf = {overall_LJ_eps_scaling} * select(protein1*dna2 + dna1*protein2, {protein_DNA_LJ_eps_scaling}, 1.0);
    ...
    ah_potential.addPerParticleParameter('protein') # protein flag
    ah_potential.addPerParticleParameter('dna') # dna flag
    ah_potential.addPerParticleParameter('anchor') # anchor flag
    ...
    coulomb_potential.addPerParticleParameter('dna') # dna flag
    coulomb_potential.addPerParticleParameter('anchor') # anchor flag
    ...
    for chain in topology.chains():
        ...
        if 'nuc' in chain.id: # as above, nucleosomes require a bit more care
            ...
            for i, atom in enumerate(atoms):
                ...
                anchor_condition = i in [41, 62, 63, 82, 115, 179, 213, 265, 268, 271, 278, 311, 390, 391, 392, 394, 395, 447, 528, 549, 550, 569, 666, 699, 700, 752, 758, 765, 798, 800, 877, 878, 879, 881, 882, 934]
                anchor = 1 if anchor_condition else 0
                protein = 1 if atom.element.symbol in ['Pt', 'Cu'] else 0
                dna = 1 if atom.element.symbol in ['Au', 'P'] else 0
                index = mapping_dict[atom.name][1]
                charge = mapping_dict[atom.name][2]
                ah_potential.addParticle([index, protein, dna, anchor])
                coulomb_potential.addParticle([charge, dna, anchor])
        else:
            for i, atom in enumerate(atoms):
                protein = 1 if atom.element.symbol in ['Pt', 'Cu'] else 0
                dna = 1 if atom.element.symbol in ['Au', 'P'] else 0
                index = mapping_dict[atom.name][1]
                charge = mapping_dict[atom.name][2]
                ah_potential.addParticle([index, protein, dna, 0])
                coulomb_potential.addParticle([charge, dna, 0])
                
    ah_potential.createExclusionsFromBonds([(bond[0].index, bond[1].index) for bond in topology.bonds()], 1)
    coulomb_potential.createExclusionsFromBonds([(bond[0].index, bond[1].index) for bond in topology.bonds()], 1)
